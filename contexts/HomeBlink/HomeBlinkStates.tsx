...
import {
  listAllHistorical,
  listAllMyFrequentsReceipts,
  listAllMyFrequentsShipments,
...
export default function HomeBlinkStates({ children }) {
  ...
  const { handleLoading, handleError, handleAccountInfoUpdated, mbUser }: any =
  ...
  const handleAmount = useCallback(async () => {
    ...
    try {
      const {
        data: {
          listMBFinancialDatas: { items: financial },
        },
      }: any = await API.graphql(
        graphqlOperation(listMBFinancialDatas, {
          filter: {
            userID: { eq: mbUser?.id },
          },
        })
      ...
      dispatch({
        type: FINANCIAL_INFO,
        payload: financial.length > 0 ? financial[0] : {},
      });
  ...
  const handlePromotionalBlinks = useCallback(async () => {
    ...
    try {
      const {
        data: {
          listMBBlinkUsers: { items: havePromotional },
        },
      }: any = await API.graphql(
        graphqlOperation(listMBBlinkUsers, {
          filter: {
            userID: { eq: mbUser?.id },
      ...
      if (havePromotional.length === 0) {
        const {
          data: {
            lastBlinkSettings: { items: blinkSettings },
          },
        }: any = await API.graphql(
          graphqlOperation(lastBlinkSettings, {
            type: "BlinkSetting",
            filter: {
        ...
        if (blinkSettings.length > 0) {
          await API.graphql(
            graphqlOperation(createMBBlinkUser, {
              input: {
                userID: mbUser?.id,
          ...
          if (state.financialID) {
            const {
              data: { updateMBFinancialData: payload },
            }: any = await API.graphql(
              graphqlOperation(updateMBFinancialData, {
                input: {
                  blinks,
            ...
            const {
              data: { createMBFinancialData: payload },
            }: any = await API.graphql(
              graphqlOperation(createMBFinancialData, {
                input: {
                  blinks: blinks,
                  type: "UserBlink",
                  amount: 0,
  ...
  const handlePaymentsMethodsByCountry = useCallback(
    async (isSend: boolean, alpha2Code: string, alpha3Code: string) => {
      handleLoading(true);
      try {
        ...
        if (isSend !== undefined && isSend !== null && !isSend) {
          AndQuery = [{ isReceipt: { eq: true } }, { isActive: { eq: true } }];
        }
        const {
          data: {
            listMBPaymentMethodCountrys: { items: payload },
          },
        }: any = await API.graphql(
          graphqlOperation(loadPaymentsMethodsByUserAndCoutries, {
            filter: {
        ...
        console.log(
          "pagos para descargar dinero desopues--",
          tempPaymentsMethod
        );
        dispatch({ payload: tempPaymentsMethod, type: SEND_PAYMENT_METHOD });
  ...
  const handleBlinkUserId = useCallback(async () => {
    ...
    try {
      ...
      dispatch({
        type: AVAILABILITY_BLINK_USER_ID,
        payload: blinks.length > 0 ? blinks[0].id : null,
      });
    } catch (e) {
      handleError(I18n.get("AN_ERROR_OCCURRED"), themeDefault.colors.error);
    } finally {
  ...
  const handleBlinkSettings = useCallback(async () => {
    ...
    try {
      ...
      const {
        data: {
          lastBlinkSettings: { items: blinks },
        },
      }: any = await API.graphql(
        graphqlOperation(lastBlinkSettings, {
          type: "BlinkSetting",
      ...
      dispatch({
        payload: blinks.length > 0 ? blinks[0] : null,
        type: CURRENT_BLINKS_SETTINGS,
      });
      handleLoading(false);
    } catch (e) {
      handleLoading(false);
  ...
  const handleFavorites = useCallback(async (isSend: boolean) => {
    ...
    try {
      ...
      if (isSend) {
        const {
          data: {
            myShipmentsFrequents: { items: payload },
          },
        }: any = await API.graphql(
          graphqlOperation(listAllMyFrequentsShipments, {
        ...
        const {
          data: {
            myRequestsFrequents: { items: payload },
          },
        }: any = await API.graphql(
          graphqlOperation(listAllMyFrequentsReceipts, {
            invitingID: mbUser?.id,
            myReceipts: { gt: 0 },
            sortDirection: ModelSortDirection.DESC,
          })
  ...
  const handleContacts = useCallback(async (searchKey: string) => {
    ...
    try {
      ...
      const {
        data: {
          listMBUsers: { items: payload },
        },
      }: any = await API.graphql(
        graphqlOperation(listMBUsers, {
          filter: {
            id: { ne: mbUser?.id },
            and: [
              {
  ...
  const handleListChargesAndTaxes = useCallback(
    async (
      alpha2Code: string,
      alpha3Code: string,
      isBlink?: boolean,
      ...
      try {
        ...
        const {
          data: {
            listMBCountrys: { items: countries },
          },
        }: any = await API.graphql(
          graphqlOperation(listChargesTaxesByCountry, {
            filter: {
              alpha2Code: { eq: alpha2Code },
              alpha3Code: { eq: alpha3Code },
            },
  ...
  const handlePurchaseBlinkPack = useCallback(async (values: any) => {
    ...
    try {
      const {
        data: { purchasePack: buyPack },
      }: any = await API.graphql(
        graphqlOperation(purchasePack, {
      ...
      if (data.statusCode === 200) {
        await handleAmount();
        handleError(
          I18n.get("BUY_PACKAGES_SUCCESSFULLY"),
          themeDefault.colors.notification
        );
  ...
  const handleTx = useCallback(async (values: any) => {
    ...
    try {
      ...
      const {
        data: { createTx: response },
      }: any = await API.graphql(
        graphqlOperation(createTx, {
          values: JSON.stringify(values),
        })
      );
  ...
  const handleLoadingHistoricalTx = useCallback(async (type?: string) => {
    ...
    try {
      ...
      if (type === "SEND") {
        orQuery = [{ shippingID: { eq: mbUser?.id } }];
      } else if (type === "REQUEST") {
        orQuery = [{ receiptID: { eq: mbUser?.id } }];
      }
      const {
        data: {
          listAll: { items: payload },
        },
      }: any = await API.graphql(
        graphqlOperation(listAllHistorical, {
          type: "TRANSACTION",
          sortDirection: ModelSortDirection.DESC,
          filter: {
            or: orQuery,
  ...
  const handleProcessBlinks = useCallback(async (values: any) => {
    ...
    try {
      ...
      const error = I18n.get(
        e?.message || e.errors[0].message || "AN_ERROR_OCCURRED"
      );
